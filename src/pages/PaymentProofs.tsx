...
export default function PaymentProofs() {
  ...
  /* import { supabase } from "@/lib/supabase"; // Ensure this is imported at top */
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const newProof = {
        ...
        // tenantName/unitName will be handled by backend/context join usually, 
        // but for optimistic UI we use current user info
        tenantName: user?.name || "Unknown",
        unitName: user?.role === 'TENANT' ? "My Unit" : "Unknown Unit",
        ...
        fileUrl: publicUrl,
        submittedAt: new Date().toISOString() // DB expects timestamp
      };

      // @ts-ignore - ID and other fields handled by DataContext/DB
      await addPaymentProof(newProof);
